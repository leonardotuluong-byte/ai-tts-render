...
# =========================================================================
# 1. LẤY DANH SÁCH GIỌNG ĐỌC (Đã sửa lỗi xung đột hệ thống trên Server)
# =========================================================================
def get_voices_sync():
    result = []
    def fetch_voices():
...
DEFAULT_LOCALE = "vi-VN" if "vi-VN" in LOCALES else (LOCALES[0] if LOCALES else "")
...
async def process_audio(input_text, voice):
    ...
    try:
        ...
        if not is_srt:
            ...
            if failed_lines:
                msg = f"⚠️ Xong, nhưng mất {len(failed_lines)} dòng."
                return temp_out.name, msg
                
            return temp_out.name, "✅ Hoàn thành xuất sắc! Âm thanh co giãn mượt mà."
...
with gr.Blocks(theme=gr.themes.Base()) as app:
    gr.Markdown("<h2 style='text-align: center; color: #22c55e;'>🎙️ AI TTS Pro - Nền Tảng Mới</h2>")
...
if __name__ == "__main__":
    # Ép ứng dụng mở cửa ở cổng 10000 để Render có thể kết nối
    app.queue().launch(server_name="0.0.0.0", server_port=10000)
